...
def detail(a):
    ...
    for i in range(7):
        ...
        # Hardening (DSC-005): bake full paid content ONLY for the US sample; every
        # other actor emits the tagged lock placeholder regardless of --input, so a
        # wrong file (e.g. actors-full) can never leak non-sample paid data. The
        # runtime hydrate fills these locks for authenticated supporters.
        if d and a['qual'][i] and a['sample']:
...
def build_page(data):
    ...
    ad_fmt=fmt_date(ad)
    ta=len(actors)  # DSC-005: derive header/snapshot count from data, never hardcode
    ...
    snap_hist = snapshot_history_html({**meta, 'total_actors': len(actors)})
...
<meta property="og:image" content="https://warmthengine.com/og-image.png">
